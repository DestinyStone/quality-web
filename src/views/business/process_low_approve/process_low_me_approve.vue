    ...
    <basic-container v-if="showMain" class="process-low-me-approve">
      ...
      <div>
        <div style="display: flex; width: 20%; min-width: 300px; margin-top: 20px;">
          <el-input v-model="query.search" placeholder="请输入事项编号/标题" size="small"></el-input>
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div style="display: flex; justify-content: space-around;">
                  <el-link :underline="false" v-if="scope.row.bpmPushStatus === 1"  type="primary" @click="handlerPutOf(scope.row)">延期</el-link>
                  <el-link :underline="false" v-if="(scope.row.bpmPushStatus === 0 || scope.row.bpmPushStatus === 2) && scope.row.processBpmStatus === 2"
                           @click="handler(scope.row)"
                           type="primary">处理</el-link>
                  <el-link type="primary" :underline="false" v-if="query.tagFlag === 1">
                    详情
                  </el-link>
                </div>
              </template>
...
<script>
  ...
  import {processLowApprovePage} from "../../../api/business/process_low/process_low";
  ...
  export default {
    ...
    data() {
      return{
        ...
        activeTag: 0,
        tagData: [
          {label: "待办", value: 0, count: 100},
          {label: "已办", value: 1, count: 100},
          {label: "本部门已超期", value: 2, count: 100},
        ],
        query: {},
        page: {
          current: 1,
